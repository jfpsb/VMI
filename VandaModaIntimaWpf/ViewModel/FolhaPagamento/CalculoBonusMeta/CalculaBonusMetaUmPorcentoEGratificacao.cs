using System;
using System.Globalization;

namespace VandaModaIntimaWpf.ViewModel.FolhaPagamento.CalculoBonusMeta
{
    public class CalculaBonusMetaUmPorcentoEGratificacao : ICalculaBonusMeta
    {
        public string NomeParaDisplay => "SE VENDEDOR BATER SUA META RECEBE 1% SOBRE VALOR VENDIDO. GRATIFICAÇÃO DE NATAL DE R$ 250,00 PARA RESTANTE.";

        public string Descricao => "Se vendedor bateu a meta de venda individual, recebe 1% do valor total vendido; senão, recebe R$ 250,00 como gratificação. Gratifica R$250,00 para funcionários em outras funções. Utilizado somente no mês de dezembro.";

        public string DescricaoBonus(DateTime mes, double totalVendido, double valorMeta)
        {
            if (valorMeta > 0 && totalVendido >= valorMeta)
            {
                return $"COMISSÃO DE VENDA - 1% - {mes.ToString("MMMM", CultureInfo.GetCultureInfo("pt-BR"))}";
            }

            return "GRATIFICAÇÃO NATAL";
        }

        public double RetornaValorDoBonus(double totalVendido, double valorMeta)
        {
            if (valorMeta > 0 && totalVendido >= valorMeta) //Vendedor
            {
                return totalVendido * 0.01; //1%
            }

            return 250;
        }
    }
}
