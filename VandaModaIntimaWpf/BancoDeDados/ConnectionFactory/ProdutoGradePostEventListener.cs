...
{
    public class ProdutoGradePostEventListener : IPostUpdateEventListener
    {
        ...
        private async void PostUpdate(PostUpdateEvent @event)
        {
            ...
            if (entity is ProdutoGrade)
            {
                var model = entity as ProdutoGrade;
                if (!model.Deletado)
                    return;
                using (IStatelessSession statelessSession = @event.Persister.Factory.OpenStatelessSession())
                {
                    using (ITransaction tx = statelessSession.BeginTransaction())
                    {
                        try
                        {
                            foreach (var c in model.SubGrades)
                            {
                                c.Deletado = true;
                                await statelessSession.UpdateAsync(c);
                            }

                            foreach (var c in model.Historico)
                            {
                                c.Deletado = true;
                                await statelessSession.UpdateAsync(c);
                            }
